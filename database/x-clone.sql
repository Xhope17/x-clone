...
-- ============================================================
--  TABLA: Permissions (cat�logo de permisos globales)
-- ============================================================
...
-- ============================================================
--  TABLA: Roles (cat�logo de roles globales del sistema)
-- ============================================================
CREATE TABLE Roles (
    Id          UNIQUEIDENTIFIER NOT NULL DEFAULT NEWID(),
    Name        NVARCHAR(100)    NOT NULL,
...
-- ============================================================
--  TABLA: UserHistory (hist�rico de eventos del usuario)
...
--  Suspension    = suspensi�n o reactivaci�n de la cuenta
--  Verification  = verificaci�n otorgada o removida
-- ============================================================
CREATE TABLE UserHistory (
    ...
    EntityName  NVARCHAR(200)    NOT NULL, -- descripci�n legible del evento
    StartedAt   DATETIME2        NOT NULL,
    EndedAt     DATETIME2        NULL,     -- NULL = a�n activo / evento puntual
    RecordedAt  DATETIME2        NOT NULL DEFAULT SYSUTCDATETIME(),
    PerformedBy UNIQUEIDENTIFIER NULL,     -- qui�n ejecut� el evento, NULL = sistema
...
VALUES
    ...
    (NEWID(), 'USERS/VERIFY',           'USERS',       'VERIFY',           'Verificar usuario',            'Permite otorgar la verificaci�n a un usuario',                  'ByAssignment'),
    -- Posts
...
VALUES
    ('USER_REGISTER',       'Registro de usuario - Xclone',        'Se cre� una cuenta con su correo electr�nico. Su contrase�a es </strong>{{password}}<strong>'),
    ('AUTH_LOGIN_SUCCESS',  'Inicio de sesi�n exitoso - XClone',   'Se inic� sesi�n en su cuenta a las <strong>{{datetime}}</strong>'),
    ('AUTH_LOGIN FAILED',   'Inicio de sesi�n fallido - XClone',   'Se intent� iniciar sesi�n en su cuenta, si no fue usted qui�n realiz� esta acci�n, comuniquese de inmediado con administraci�n'),
	('AUTH_REGISTER_EMAIL_VERIFICATION', 'Verifica tu cuenta - XClone', 'Hola, para continuar con su proceso de registro, necesita validar su correo electr�nico haciendo clic en el siguiente <a href="{{url}}">enlace</a>.'),
	('AUTH_RECOVER_PASSWORD_OTP', 'C�digo de recuperaci�n de contrase�a - XClone', 'Hola, el siguiente c�digo te permitir� restablecer tu contrase�a para volver a entrar a tu cuenta: <strong>{{otp}}</strong>. No compartas este c�digo con nadie.');
...
CREATE TABLE MenuPermissions (
    ...
    PermissionId UNIQUEIDENTIFIER NOT NULL,
    -- ALL = el usuario debe tener TODOS los permisos del men� para verlo
...
VALUES
    -- Men�s ra�z (visibles para todos los usuarios)
    ...
    -- Men� de administraci�n (solo Admin y Moderator)
    (@IdAdmin,         'admin',                 'Administraci�n',   '/admin',                   'shield',         NULL,            7,  1, 1),
...
WHERE
    ...
    -- Panel de admin: requiere al menos un permiso de gesti�n de usuarios o contenido
    OR (m.Code = 'admin'              AND p.Code IN ('USERS/DISABLE', 'USERS/VERIFY', 'POSTS/DELETE_ANY', 'POSTS/MARK_SENSITIVE'))
    -- Gesti�n de usuarios: suspender o verificar cuentas
    OR (m.Code = 'admin.users'        AND p.Code IN ('USERS/DISABLE', 'USERS/VERIFY', 'USERS/UPDATE', 'USERS/CREATE'))
    -- Gesti�n de contenido: moderar posts, replies y comunidades
